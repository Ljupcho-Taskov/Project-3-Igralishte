...
const ProductItem: React.FC<ProductItemProps> = ({
  ...
  const discountAmount = (discountProcent / 100) * parseInt(price);
  const discountedPrice = parseInt(price) - discountAmount;
  ...
  const handleToggleFavorite = (
    ...
    if (isFavorite) {
      removeFromFavorites({ id, img, title, price, discount, discountProcent });
    } else {
      addToFavorites({ id, img, title, price, discount, discountProcent });
    }
  };

  return (
    <div
      className="col-6 pointer mb-3"
      key={id}
        ...
        <div>
          ...
          <div>
            <div className="my-1 d-flex justify-content-between">
              ...
              {discount ? (
                <p className="productItemTitle discountP">
                  {discountProcent}% off
                </p>
              ) : null}
            </div>
            <div className="d-flex justify-content-between">
              <p
                className={`productItemTitle ${
                  discount ? "red-alert line-through" : ""
                }`}
              >
                {price} ден.
              </p>
              {discount ? (
                <p className="productItemTitle">{discountedPrice} ден.</p>
              ) : null}
            </div>
          </div>
        </div>
